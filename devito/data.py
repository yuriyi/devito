...
class Data(np.ndarray):
    ...
    :param glb_to_loc: (Optional) a mapper from :class:`Dimension`s in ``dimensions`
                       to functions that translate (user-provided) global array
    ...
    def __getitem_local__(self, index):
        """Like __getitem__, but treat the index as a local index"""
        return super(Data, self).__getitem__(index)
